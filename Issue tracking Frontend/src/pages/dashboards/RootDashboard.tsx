...
const RootDashboard = () => {
  ...
  // Handle initial tab load and hash changes
  useEffect(() => {
    const updateTabFromHash = () => {
      ...
      if (['registration', 'issues', 'roles', 'review'].includes(hash)) {
        ...
        // Default to 'issues' if no valid hash
    ...
    return () => {
      window.removeEventListener('popstate', updateTabFromHash);
    };
  ...
  const handleTabClick = (tabKey: string) => {
    ...
    if (activeTab !== tabKey) {
      ...
      navigate(`${location.pathname}#${tabKey}`, { replace: true });
    }
  };
